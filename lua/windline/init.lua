...
M.show_normal = function(bufnr, winid)
	...
	if vim.g.statusline_winid ~= cur_win then
		-- in active
		if M.last_win == vim.g.statusline_winid and api.nvim_win_get_config(cur_win).relative ~= "" then
...
M.show_global = function(bufnr, winid)
	...
	if vim.g.statusline_winid == winid then
		if
			...
			or (api.nvim_win_get_config(winid).relative ~= "" and not check_line.global_show_float)
		then
...
M.on_win_enter = function(bufnr, winid)
	...
	vim.api.nvim_win_set_option(winid, "statusline", string.format("%%!v:lua.WindLine.show(%s,%s)", bufnr, winid))
			...
			if not winbar.enable or winbar.enable(bufnr, winid) and i_winid == winid then
				if vim.api.nvim_win_get_config(winid).relative == "" then
					vim.api.nvim_win_set_option(
						winid,
						"winbar",
						string.format("%%!v:lua.WindLine.show_winbar(%s,%s)", bufnr, winid)
					)
				end
			else
...
M.get_colors = function(reload)
	if not reload and M.state.colors then
		return M.state.colors
	end
...
M.setup = function(opts)
	...
	if M.state.floatline then
		WindLine.floatline_disable()
	end
	if M.anim_reset then
		M.anim_reset()
	end
