...
M.lsp_progress = function(opts)
  ...
  local function handler(_, msg, info)
    ...
    if token then
      ...
      local pregress = client.progress[token]
      if val then
        if val.kind == 'begin' then
          pregress.message = opts.message.commenced
          pregress.title = val.title and (val.title .. ' ') or ''
        elseif val.kind == 'report' then
          pregress.message = progress.title .. val.message
          pregress.percentage = val.percentage or 0
        elseif val.kind == 'end' then
          pregress.percentage = 100
          pregress.hrtime = vim.loop.hrtime()
          pregress.is_done = true
          pregress.message = opts.message.completed
        end
      end
    end
  end
