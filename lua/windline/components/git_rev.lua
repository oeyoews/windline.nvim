...
M.git_rev_update = function()
	uv_utils.uv_run("git fetch", function()
		uv_utils.uv_run("git rev-list --count --left-right @{upstream}...HEAD", function(data)
			if data then
	...
	return cache_utils.one_call_func("git_rev", function()
		...
		return function()
			...
			if rev and (rev[1] > 0 or rev[2] > 0) then
				return string.format(opt.format, rev[opt.format_index[1]], rev[opt.format_index[2]])
			end
			return ""
		end
	end)
