...
function Comp:make_hl(hl)
	...
	if type(hl) == "string" then
		...
		last_group = self.is_not_group and self.hl_colors or self.hl_colors and self.hl_colors[hl] or last_group
...
function Comp:render(bufnr, winid, width)
	...
	if type(childs) == "table" then
		...
		for _, child in pairs(childs) do
			...
			result = result .. render_click(render_text(text, self:make_hl(hl)), child[3])
		end
